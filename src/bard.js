class Bard {
  ...
  async #query(message, config) {
    ...
    // Text of that answer
    const stringToReplace = '\\[Image of |\\]';
    const replacementString = '**';
    const text = answer[1][0];
    const text1 = text.replace(new RegExp(stringToReplace, 'gi'), replacementString);


    // Get data about images in that answer
    const images =
      answer[4]?.map((x) => (x[3][0][0])) ?? [];

    this.#verbose && console.log("✅ All done!\n");
    // Put everything together and return
    return {
      content: formatMarkdown({ message: text1, imageUrls: images }),
      imageUrls: images,
      ids: {
